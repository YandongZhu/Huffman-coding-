...
void read(){
	...
	for(int i = 0; i < leap; i++){
		input = cin.get(); // get the next character
		if (cin.eof()) {
			throw("did't read enough integer.");
			break;
			} // if there is eof the break;
		frequency[input]++; // else increase the frequency at position input
		cout << code[input]; // output the corresponding code
	}
	//cout << "13 = " << frequency[13] << endl;
}
...
{
	ofstream freq ("huff.freq");
	ofstream finalCode ("huff.code");
	ofstream wpl ("huff.wpl");
	// initial the T0
	initial();
	...
	for (int i = 0; i < length; ++i)
	{
		finalCode << code[i] << endl;
	}
	for (int i = 0; i < length; ++i)
	{
		freq << frequency[i] << endl;
	}
	int sum = 0;
	if (leap == 128){
		wpl << 896 << endl;
		...
		for (int i = 0; i < length; ++i)
		{
			int curLen = code[i].length();	
			int curFreq = frequency[i];
			sum = sum + curLen * curFreq;
		}
		wpl << sum << endl;
	}
	return 0;
